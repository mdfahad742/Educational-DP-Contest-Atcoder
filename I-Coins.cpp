...
void solver() {
    ...
    vector<db> a(n);
    forn(i, n)cin >> a[i];
    db dp[n + 1][n + 1]; // dp[i][j] - probab that out of i coins, exactly j coins are head.

    forn(i, n + 1) {
        forn(j, n + 1)dp[i][j] = 0.0;
    }
    ...
    forn(i, n + 1) {
        forn(j, i + 1) {
            ...
            else if (j == 0) {dp[i][j] = dp[i - 1][j] * (1.0 - a[i - 1]);}
            else {
                dp[i][j] = (i - 1 >= 0 and j - 1 >= 0) ? (dp[i - 1][j - 1] * a[i - 1]) : 0;

                dp[i][j] += (i - 1 >= 0) ? dp[i - 1][j] * (1 - a[i - 1]) : 0.0;
